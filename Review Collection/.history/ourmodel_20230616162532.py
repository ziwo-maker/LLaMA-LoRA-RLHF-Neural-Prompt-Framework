...
def main(
    ...
    lora_weights: str = "gogger/yg-llm-13b",
    ...
    def do_c3():
        ...
        for i in data_all:
            ...
            s=s+i[1][0]['question']+"choice:"+choices


            for t in range(len(i[0])):
                input=input+i[0][t]
            instruction=s
            tmp=get_value(instruction,input=input)
            ans.append(tmp)
    ...
    def do_lcsts():
        ...
        ans=[]
        for i in data_all:
            instruction="请将这句话提取出一个标题";

            tmp=get_value(instruction,input=i["content"])
            tmp=tmp+i['title']
            ans.append(tmp)
        with open(path2,'w') as f:
            json.dump(ans,f);
    ...
    def do_CINLID():
        ...
        with open (filename,'r',encoding="utf-8") as f:
            for i in range(0,count):
                ...
                s='下面两个成语是什么关系'+'choice:[相似，"相反"，不相关]'
                instruction=s
                input=data_line[0]+','+data_line[1]
                tmp=get_value(instruction,input=input)
                print(tmp)
